...
const { dbConfig, UPLOAD_DIR } = require('./load-env');
...
(async () => {
  ...
  try {
    ...
    console.log('\n-- CHAT --');
    check('SEGURIDAD: chat sin token -> 401', (await req('GET', `/chat/getAllChats/${teamId}`)).status === 401, 'no dio 401');
    check('SEGURIDAD: extraño no lee el chat del equipo -> 403',
      (await req('GET', `/chat/getAllChats/${teamId}`, { token: tok.outsider })).status === 403, 'no dio 403');
    check('SEGURIDAD: extraño no escribe en el chat del equipo -> 403',
      (await req('POST', `/chat/sendMessage/${teamId}`, { token: tok.outsider, form: { message: 'hack' } })).status === 403, 'no dio 403');
    check('sendMessage', (await req('POST', `/chat/sendMessage/${teamId}`, { token: tok.member, form: { message: 'hola phpit' } })).status === 200, 'no dio 200');

    const ch = await req('GET', `/chat/getAllChats/${teamId}`, { token: tok.member });
    ...
    await req('POST', `/chat/sendMessage/${teamId}`, { token: tok.member, form: { username: 'jefe@suplantado.com', message: 'phpit-suplantacion' } });
    const ch2 = await req('GET', `/chat/getAllChats/${teamId}`, { token: tok.member });
    ...
    // Cada miembro del equipo (menos quien escribe) debe recibir una
    // notificación in-app de que hay mensaje nuevo en el chat del equipo.
    const nfChatLeader = await req('GET', '/notifications', { token: tok.leader });
    check('el resto del equipo recibe notificacion del mensaje de chat',
      nfChatLeader.body?.notifications?.some((n) => n.type === 'message_received' && n.teamId === teamId),
      `${nfChatLeader.text.slice(0, 250)}`);
    const nfChatMember = await req('GET', '/notifications', { token: tok.member });
    check('quien envia el mensaje no se auto-notifica',
      !nfChatMember.body?.notifications?.some((n) => n.type === 'message_received' && n.message?.includes('hola phpit')),
      `${nfChatMember.text.slice(0, 250)}`);
