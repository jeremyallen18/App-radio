...
const { loadEnv, dbConfig, UPLOAD_DIR } = require('./load-env');
...
// Credenciales y ruta de uploads leídas de hive-backend/.env (antes estaban
// hardcodeadas acá con los valores de ejemplo de .env.example, que no son
// los reales del proyecto).
const env = loadEnv();
const UPLOADS = UPLOAD_DIR;

const targets = [
  { label: `hive_db (backend PHP, el que usa la app — leído de ${path.relative(process.cwd(), env.envPath)})`, config: dbConfig() },
];
