...
// Recorta un mensaje para usarlo como vista previa en una notificación.
// Antes se usaba mb_strlen()/mb_substr() directamente, pero esas funciones
// requieren la extensión `mbstring`, que no siempre está habilitada (p. ej.
// en instalaciones locales de PHP sin esa extensión activada). Cuando
// faltaba, cualquier envío de mensaje (chat de equipo, chat privado o
// mensaje al líder) terminaba en un error 500 justo después de guardar el
// mensaje, aunque este sí quedaba guardado. Aquí se usa mb_* solo si está
// disponible y si no, se cae de vuelta a strlen()/substr() normales, que
// para un simple recorte de vista previa es suficiente.
function text_preview(string $text, int $maxLength = 80): string {
